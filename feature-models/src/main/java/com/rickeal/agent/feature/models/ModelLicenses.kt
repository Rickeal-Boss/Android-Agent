 ...
 *
 * `ModelFamily` 现在已经能认出 Gemma 3 / 4（`:core-model` 里有 `GEMMA_3N` / `GEMMA_3` /
 * `GEMMA_4`，`inferFamily` 也能命中 `gemma-4-*`），但 family **仍然不适合**做授权闸门，
 * 原因有二：
 *
 * 1. **误报方向更安全**：字符串判据是「宁可多问、不可漏问」——误报只是多问一次，
 *    漏报则是没拿到授权就用。family 一旦判错，方向恰恰是漏拦（危险方向）。
 * 2. **family 只覆盖已知启发式**：`inferFamily` 是按文件名里少数几个已知模式匹配的，
 *    用户 SAF 导入的任意 Gemma 文件（例如 `gemma-2-*.litertlm`，或自己改过名的文件）
 *    会落进 `OTHER`，用 family 做闸门同样会漏拦。
 *
