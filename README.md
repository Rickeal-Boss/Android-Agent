...
> **⚠️ 活跃开发在 [`harness`](https://github.com/Rickeal-Boss/Android-Agent/tree/harness) 分支** —— 最新进度、纯端侧架构（远程后端已整体移除）与全部新特性都在那边；本分支是早期快照，下文路线图与依赖描述可能滞后。
>
...
- **M7（已从规划移除）** — ~~远程后端：OkHttp + SSE~~ 项目已收敛**纯端侧**，远程后端与 okhttp 依赖整体删除（见 harness 分支）。
